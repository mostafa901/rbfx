...
bool RadianceMap::FillInvalidPixels(int searchDistance)
{
    ...
    ea::vector<Vector2> coords;

    // left
    coords.push_back(Vector2(-searchDistance, 0));
    // right
    coords.push_back(Vector2(searchDistance, 0));

    // down
    coords.push_back(Vector2(0, searchDistance));
    // up
    coords.push_back(Vector2(0, -searchDistance));

    coords.push_back(Vector2(-searchDistance, -searchDistance));
    coords.push_back(Vector2(searchDistance, searchDistance));
    coords.push_back(Vector2(-searchDistance, searchDistance));
    coords.push_back(Vector2(searchDistance, -searchDistance));
    ...
    for (int x = 0; x < width; x++)
    {
        ...
        {
            Color c;
            ea::unordered_map<int, ea::vector<Color>> colors;
            ...
            if (!CheckValidPixel(x, y , c))
            {
                ...
                if (colors.size())
                {
                    ...
                    while (itr != colors.end())
                    {
                        ...
                        if (itr->second.size() > bestCount)
                        {
                            bestCount = itr->second.size();
                            bestTri = itr->first;
                        }
                    ...
                    const ea::vector<Color>& triColors = colors[bestTri];
